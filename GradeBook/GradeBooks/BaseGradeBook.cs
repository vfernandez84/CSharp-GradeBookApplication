...
namespace GradeBook.GradeBooks {
    public abstract class BaseGradeBook {
        ...
        public virtual double GetGPA(char letterGrade, StudentType studentType) {
            bool applyWeight = IsWeighted && (studentType == StudentType.Honors || studentType == StudentType.DualEnrolled);
            switch (letterGrade) {
                case 'A':
                    return applyWeight == true ? 5 : 4;
                case 'B':
                    return applyWeight == true ? 4 : 3;
                case 'C':
                    return applyWeight == true ? 3 : 2;
                case 'D':
                    return applyWeight == true ? 2 : 1;
                case 'F':
                    return applyWeight == true ? 1 : 0;
            }
            return 0;
        }
